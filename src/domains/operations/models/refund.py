...
class RefundModel(BaseModel):
    __tablename__ = "operations_refunds"

    transaction_id: Mapped[str] = mapped_column(String(255), nullable=False, index=True, unique=True)
    transaction_type: Mapped[str] = mapped_column(String(100), nullable=False, default="REFUND")
    
    order_reference: Mapped[str] = mapped_column(String(255), nullable=False, index=True)
    ...
    refund_captured_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
    ...
    settlement: Mapped[Optional["SettlementModel"]] = relationship("SettlementModel")
