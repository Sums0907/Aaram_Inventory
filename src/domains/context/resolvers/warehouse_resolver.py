...
class WarehouseSemanticResolver:
    ...
    async def resolve(self, semantic_value: Any, target_type: str) -> EntityResolutionResult:
        if target_type != "UUID":
            return EntityResolutionResult(
                ...
                error_reason=f"Target type {target_type} is not supported by Warehouse resolver"
            )

        val_uuid = try_parse_uuid(semantic_value)
        if val_uuid:
            stmt = select(WarehouseModel.id).where(WarehouseModel.id == val_uuid)
        else:
            val = str(semantic_value)
            stmt = select(WarehouseModel.id).where(
                or_(
                    WarehouseModel.warehouse_code == val,
                    WarehouseModel.warehouse_name == val
                )
            )
        ...
        if len(rows) == 0:
            return EntityResolutionResult(
                ...
                original_value=semantic_value,
                target_type=target_type,
                resolver_provenance="WarehouseSemanticResolver"
            )
        elif len(rows) > 1:
            return EntityResolutionResult(
