...
class SupplierSemanticResolver:
    ...
    async def resolve(self, semantic_value: Any, target_type: str) -> EntityResolutionResult:
        if target_type != "UUID":
            return EntityResolutionResult(
                ...
                semantic_identity="inventory.entity.supplier",
                original_value=semantic_value,
                target_type=target_type,
                error_reason=f"Target type {target_type} is not supported by Supplier resolver"
            )
        ...
        if len(rows) == 0:
            return EntityResolutionResult(
                ...
                original_value=semantic_value,
                target_type=target_type,
                resolver_provenance="SupplierSemanticResolver"
            )
        elif len(rows) > 1:
            return EntityResolutionResult(
