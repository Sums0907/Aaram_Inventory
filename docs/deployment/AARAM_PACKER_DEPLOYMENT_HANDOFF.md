...
1. **SQLite to PostgreSQL Migration (Codebase Parity):** If the local Packer application is currently running on SQLite, you must first update the codebase to support PostgreSQL. Ensure `asyncpg` or `psycopg2-binary` is added to `requirements.txt`, update SQLAlchemy to read from the `DATABASE_URL` dynamically, and verify Alembic migrations are compatible with Postgres before proceeding.
2. **Frontend CI/CD Bypass:** Do not attempt to build the Vite/React frontend using Docker on GitHub Actions (it will silently timeout/fail). You must pre-build the frontend locally (`npm run build`), force-add `dist/` to git (`git add -f frontend/dist`), and update the frontend Dockerfile to simply `COPY dist/ /usr/share/nginx/html`.
3. **Database Preparation:** SSH into the VPS and manually create a dedicated database and user (`packer_user` / `packer_prod`) via `sudo -u postgres psql`. Do not use the default `postgres` superuser. (The Docker subnet `172.16.0.0/12` is already whitelisted in `pg_hba.conf`).
